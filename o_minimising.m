%Zhi Zhang, 16,07,2023, Newcastle University
%This code is for O-minimising, and is to fix the allocation of the upper and lower transient
%probability with respect to the transient initated from a state q to the
%rest of states so that the minimum of formula
%\sum_{i=1}^{r_o-1}\hat{P}(q,o_i)+\sum_{i=r_o+1}^{|Q|}\breva{P}(q,o_{i})
%****Important: P_min is the vector consisted of the allocated lower and
%upper transient probability. a is the action which can achieve the minimum
%value of the above equation.


function P_min = o_minimising (transipro_lo_poin,transipro_up_poin,index,numb_Act)
%r_oThis is counter used to the iteration to obtain value r_o which 
...
%         if P_ro<transi_order_lo(:,r_o)
%             P_ro=transi_order_lo(:,r_o);
%         end
...
%         end
        ...
        for i_judge=1:numb_Act
            if judge1(i_judge)<judge2(i_judge) %if this iteration is smaller than the last iteration
...
%P_min = o_miximising (transipro_lo_poin,transipro_up_poin,index,numb_Act);
