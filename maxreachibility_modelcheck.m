...
%****action series: is the action required to achieve the maximum
...
%within k steps.
function [p_max,action_series]=maxreachibility_modelcheck(M_lo,M_up,Act,S_total,S_targ,S_nev,q_ini,k)
eplis=10^(-5);
                ...
                for i_row=1:row
                    for i_col=1:col
                        ...
                        elseif min(abs(point_targ-S_nev),[],'all')<=eplis
                            ...
                            %reordering based on the O-maximising
                            [buff1,index]=sort(buff0,2,'descend');%since the midd_store is a vector,
                            %****buff1 is the reordered vector statify the dscending with
                            %respect to the probability of target satisfication. Index is
                            %the order of elements of buff0. ****Important: the buff0 is
                            %the maximum transient probility of last step.
                            ...
                            MiddP_max = o_maximising(transipro_lo_poin,transipro_up_poin,index,numb_Act);%The middle probability vector to
                            ...
                            for i_zero=1:numb_Act
                                if Find1_whole0(i_zero)==0
                                    MiddP_max(i_zero,:)=-1.5*ones(1,col);%This line means replacing
                                    %the whole zero elements in a row by very small value
                                end
                            end
                            %%%%
                            
                            [Pro_max,action]=max(MiddP_max*buff1');%Pro_max is the maximum probability and action is the relevant action
                            %%put Pro_max into the buff0
                            buff0(i_row,i_col)=Pro_max;
