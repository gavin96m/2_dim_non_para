% data 5*10^4
% x,y [0-2]


tic
filename = "data_1000";
get_2step(1000,filename);
dim2_handle(filename,'result_1000_by_point_25_100',5);
elapsed_time_1000 = toc;
fprintf('Time taken for first block: %f seconds\n', elapsed_time_1000);


% tic
% filename = "data_1000";
% get_2step(1000,filename);
% dim2_handle_by_point(filename,'result_1000_by_point_25_100',5,10);
% elapsed_time_1000 = toc;
% fprintf('Time taken for first block: %f seconds\n', elapsed_time_1000);
% 
% tic
% filename = "data_1000";
% get_2step(1000,filename);
% dim2_handle_by_point(filename,'result_1000_by_point_25_400',5,20);
% elapsed_time_1000 = toc;
% fprintf('Time taken for second block: %f seconds\n', elapsed_time_1000);
% 
% tic
% filename = "data_2000";
% get_2step(2000,filename);
% dim2_handle_by_point(filename,'result_2000_by_point_25_100',5,10);
% elapsed_time_2000 = toc;
% fprintf('Time taken for third block: %f seconds\n', elapsed_time_2000);
% 
% tic
% filename = "data_2000";
% get_2step(2000,filename);
% dim2_handle_by_point(filename,'result_2000_by_point_25_400',5,20);
% elapsed_time_2000 = toc;
% fprintf('Time taken for fourth block: %f seconds\n', elapsed_time_2000);
% 
% tic
% filename = "data_1000";
% get_2step(1000,filename);
% dim2_handle_by_point(filename,'result_1000_by_point_400_400',20,20);
% elapsed_time_1000 = toc;
% fprintf('Time taken for fivth block: %f seconds\n', elapsed_time_1000);
% 
% tic
% filename = "data_2000";
% get_2step(2000,filename);
% dim2_handle_by_point(filename,'result_2000_by_point_400_400',20,20);
% elapsed_time_2000 = toc;
% fprintf('Time taken for sixth block: %f seconds\n', elapsed_time_2000);


disp('Everything is done.');
